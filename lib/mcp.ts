...
export class ResumeMcpServer {
  ...
  private registerToolsOnServer(server: McpServer) {
    const data = this.loadData();
    server.tool("get_bio", {}, async () => ({
      content: [
        { type: "text", text: JSON.stringify(data?.bio || {}, null, 2) },
      ],
    }));
    server.tool("get_jobs", {}, async () => ({
      content: [
        { type: "text", text: JSON.stringify(data?.jobs || {}, null, 2) },
      ],
    }));
    server.tool("get_education", {}, async () => ({
      content: [
        { type: "text", text: JSON.stringify(data?.education || {}, null, 2) },
      ],
    }));
    server.tool("get_skills", {}, async () => ({
      content: [
        { type: "text", text: JSON.stringify(data?.skills || {}, null, 2) },
      ],
    }));
    server.tool("get_publications", {}, async () => ({
      content: [
        {
          type: "text",
          text: JSON.stringify(data?.publications || {}, null, 2),
